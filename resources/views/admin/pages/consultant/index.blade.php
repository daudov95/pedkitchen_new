                                        ...
                                        @foreach ($consultants as $consultant)
                                            <tr class="{{ $loop->odd ? 'odd' : 'even' }}">
                                                <td class="dtr-control sorting_1" tabindex="0" colspan="6">
                                                    <a href="{{ route('admin.consultant.edit', ['id' => $consultant->id]) }}">{{ $consultant->name }}</a>
                                                </td>
                                                
                                                <td>
                                                    <div class="table-action">
                                                        <a href="{{ route('admin.consultant.edit', ['id' => $consultant->id]) }}"  class="btn btn-block btn-outline-primary table-action__btn-edit"><i class="nav-icon fas fa-edit"></i></a>
                                                        <form action="{{ route('admin.consultant.delete') }}" class="delete-route" method="POST">
                                                            @CSRF
