...
@section('page_title') Все подразделы @endsection
    ...
    <div class="col-12">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Все подразделы</h3>
            </div>
                                        ...
                                        @foreach ($subsections as $section)
                                            <tr class="{{ $loop->odd ? 'odd' : 'even' }}">
                                                <td class="dtr-control sorting_1" tabindex="0" colspan="6">
                                                    <a target="_blank" href="{{ route('posts.subcategory', ['category' => $section->parent_id, 'subcategory' => $section->id]) }}">{{ $section->title }}</a>
                                                </td>
                                                
                                                <td>
                                                    <div class="table-action">
                                                        <a href="{{ route('admin.section.subedit', ['id' => $section->id]) }}"  class="btn btn-block btn-outline-primary table-action__btn-edit"><i class="nav-icon fas fa-edit"></i></a>
                                                        <form action="{{ route('admin.section.subdelete') }}" class="delete-route" method="POST">
                                                            @CSRF
