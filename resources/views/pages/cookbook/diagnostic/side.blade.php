<div class="archive__left">
    ...
    @if(isset($menu))
        <ul class="archive-menu">
            ...
            <li class="archive-menu__item"><a href="{{ route('cookbook.diagnostic') }}" class="archive-menu__link {{ !isset($category) ? 'active' : '' }}" style="background-image: url({{ asset('assets/img/archive/icons/icon1.png') }})">Менюборд</a></li>
            ...
            @foreach ($menu as $item)
                ...
                <li class="archive-menu__item">
                    ...
                    <a href="{{ route('cookbook.category.diagnostic', ['category'=> $item->id]) }}" class="archive-menu__link {{ isset($category) && isset($post) && $item->id == $post->category_id ? 'active': '' }} {{ isset($category) && $item->id == $category->id ? 'active': '' }}" style='background-image:url("{{ asset("storage/image/icon-monograph.png") }}")'>
                        {{$item->title}}
                    </a>
                </li> 
            @endforeach
